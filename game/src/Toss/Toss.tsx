    ...
    return (
        <div style={{ backgroundImage: `${result === "WON" ? `url('https://media4.giphy.com/media/26BRABnerqonwLHMc/200w.webp?cid=ecf05e47pychi2lxj02ufzmextf3xmx78i3fsc8f88fc0p1f&rid=200w.webp&ct=s')` : `url('')`}`, backgroundSize: "fit" }} className={`border-0 w-3/4 p-3 rounded-lg mx-auto backdrop-grayscale-0 bg-white/70`} >
            ...
            </div>
            {
                !spinner && (
                    <div className='mx-auto mt-6 w3/4'>
                        ...
                        <div className="mx-auto my-4 grid grid-cols-2 p-3 justify-center w-1/4">
                            {
                                outcomes.map(e => (
                                    <button
                                        key={e}
            ...
            }
            {
                toss && result && (
                    <div className='m-0'>
                        <p className='text-center antialiased font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-pink-400 to-red-600 tracking-tight text-4xl dark:text-white p-6'>
                            YOU CHOOSE : {toss}</p>
